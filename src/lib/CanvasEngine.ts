...
export class CanvasEngine {
  ...
  private _dispatchToWorker = throttle((params: GradingState) => {
    ...
    const workerParams = {
      ...
      midtones: {
        r: Math.pow(
          2,
          params.primary.midtones.luma -
            1.0 +
            params.primary.midtones.x -
            params.primary.midtones.y / 2,
        ),
        g: Math.pow(
          2,
          params.primary.midtones.luma -
            1.0 -
            params.primary.midtones.x / 2 -
            params.primary.midtones.y / 2,
        ),
        b: Math.pow(
          2,
          params.primary.midtones.luma -
            1.0 -
            params.primary.midtones.x / 2 +
            params.primary.midtones.y,
        ),
      },
      highlights: {
        r:
          params.primary.highlights.luma +
          params.primary.highlights.x -
          params.primary.highlights.y / 2,
        g:
          params.primary.highlights.luma -
          params.primary.highlights.x / 2 -
          params.primary.highlights.y / 2,
        b:
          params.primary.highlights.luma -
          params.primary.highlights.x / 2 +
          params.primary.highlights.y,
      },
  ...
  public async loadImage(
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        this._histogramProxyImageData = histCtx.getImageData(
          0,
          0,
          histSize,
          histSize,
  ...
  public render(
    ...
    gl.uniform1i(
      this._uniforms["u_showShadowClipping"],
      histState.showShadowClipping ? 1 : 0,
    );
    gl.uniform1i(
      this._uniforms["u_showHighlightClipping"],
  ...
  public updateCurveTextures(curves: CurvesState) {
    ...
    channels.forEach((ch) => {
      ...
      const label =
        (ch as string).charAt(0).toUpperCase() + (ch as string).slice(1);
  ...
  public exportHighRes(
    ...
    hslAttrs.forEach((attr, i) => {
      ...
      for (let x = 0; x < 256; x++) {
        ...
        let val = 0;
        weights.forEach((w) => {
          val += params.hsl[w.bin as keyof HSLState][attr] * w.weight;
        });
    ...
    // LUT
    const exportLutEnabled = this._lutData !== null;
    setUni("u_lutEnabled", "uniform1i", exportLutEnabled ? 1 : 0);
    const lutTex = gl.createTexture();
    gl.activeTexture(gl.TEXTURE8);
    gl.bindTexture(gl.TEXTURE_3D, lutTex);
    if (exportLutEnabled && this._lutData) {
      const lutStrength = (store.getState() as any).lut?.strength ?? 100;
      gl.texImage3D(
        gl.TEXTURE_3D,
        0,
      ...
      gl.texParameteri(gl.TEXTURE_3D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
      setUni("u_lut", "uniform1i", 8);
      setUni("u_lutStrength", "uniform1f", 0.0);
      setUni("u_lutSize", "uniform1i", 33);
    }
